...
def get_images_info_and_annotations(opt):
    ...
    # BAO
    if path.is_dir():
        file_paths = sorted(path.rglob("*.txt"))
    else:
        with open(path, "r") as fp:
            read_lines = fp.readlines()
        file_paths = [Path(line.replace("\n", "")) for line in read_lines]
    ...
    for file_path in tqdm(file_paths):
        ...
        file_path = Path(str(file_path).replace("/YOLO_darknet", "").replace(".txt",".png"))
...
def get_args():
    ...
    parser.add_argument(
        "--yolo-subdir",
        default="/home/dhgbao/Machine_Learning/SpineXR/vindr-spinexr-dataset/annotations/YOLO_darknet",
        action="store_true",
